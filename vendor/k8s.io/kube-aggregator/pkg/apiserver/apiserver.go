...
func (c completedConfig) NewWithDelegate(delegationTarget genericapiserver.DelegationTarget) (*APIAggregator, error) {
	...
	// Group"apiregistration.k8s.io"下只有"APIService"这个API Object和他的子object status
	...
	// 每一个APIService在ETCD中被创建出来，都需要被Aggregator考虑进来从而能代理他所提供的服务
	// 有更新的时候调用 AddAPIService 和 RemoveAPIService 方法
	apiserviceRegistrationController := NewAPIServiceRegistrationController(informerFactory.Apiregistration().V1().APIServices(), s)
	if len(c.ExtraConfig.ProxyClientCertFile) > 0 && len(c.ExtraConfig.ProxyClientKeyFile) > 0 {
		// 当证书变更时，所有APIService要重新入队上一步建出来的controller，去重建一下
...
func (s *APIAggregator) PrepareRun() (preparedAPIAggregator, error) {
	...
	// openapi、healthz、livez、readyz等endpoints
