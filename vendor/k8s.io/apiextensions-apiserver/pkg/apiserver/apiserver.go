...
func (c completedConfig) New(delegationTarget genericapiserver.DelegationTarget) (*CustomResourceDefinitions, error) {
	...
	// 创建apiGroupInfo，groups是apiextensions.k8s.io,version是v1,resource是customresourcedefinitions
	...
	// 请求 /apis/groups/version返回的内容，就是个版本发现的功能
	versionDiscoveryHandler := &versionDiscoveryHandler{
		discovery: map[schema.GroupVersion]*discovery.APIVersionHandler{},
		delegate:  delegateHandler,
	}
	// 请求 /apis/groups返回的内容，就是个组发现的功能
	groupDiscoveryHandler := &groupDiscoveryHandler{
		discovery: map[string]*discovery.APIGroupHandler{},
		delegate:  delegateHandler,
	}
	establishingController := establish.NewEstablishingController(s.Informers.Apiextensions().V1().CustomResourceDefinitions(), crdClient.ApiextensionsV1())
	// 自定义的对象也是 /apis开头的请求，genericserver是处理不了，只能自己去处理这些请求
	...
	// 验证数据是否符合openapi v3的schema，更新status
	nonStructuralSchemaController := nonstructuralschema.NewConditionController(s.Informers.Apiextensions().V1().CustomResourceDefinitions(), crdClient.ApiextensionsV1())
	// 创建k8s.io namespace下的CR，需要设置annotation  api-approved.kubernetes.io
