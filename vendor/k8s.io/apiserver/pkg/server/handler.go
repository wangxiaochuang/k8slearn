...
func NewAPIServerHandler(name string, s runtime.NegotiatedSerializer, handlerChainBuilder HandlerChainBuilderFn, notFoundHandler http.Handler) *APIServerHandler {
	...
	// 这个包含了处理restful和非restful的mux，有ServeHTTP方法
	// 会遍历注册的每一个webservice，找/apis开头的路径，并处理
	// 处理完了后交给非restful的handler进行处理
	director := director{
		name:               name,
	...
	// 最后再包装了下，这个也有ServeHTTP方法，很简单，就是交给FullHandlerChain来处理
	return &APIServerHandler{
		// 这里通过装饰器，添加前序的登陆鉴权等通用功能
