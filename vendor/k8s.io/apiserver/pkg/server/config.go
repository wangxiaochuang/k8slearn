...
func (c completedConfig) New(name string, delegationTarget DelegationTarget) (*GenericAPIServer, error) {
	...
	// 这就是个包装器，所有请求处理之前要处理的内容都在这里
	// 这个BuildHandlerChainFunc在config创建的时候，设置了DefaultBuildHandlerChain
	handlerChainBuilder := func(handler http.Handler) http.Handler {
		return c.BuildHandlerChainFunc(handler, c.Config)
	}

	// 这是GenericServer的handler创建函数
	// UnprotectedHandler 这个函数就只是返回了Handler的Director，不用进行认证和授权，因为aggregator已经做过了
	...
	s := &GenericAPIServer{
		...
		// 这里就是处理请求的Handler，连接了url和响应函数；同时包含每隔request需处理的预处理函数
		...
		// 启动前后的一些hook点，这些hook点都是其他server加进来的
		// 另外一部分来源是下一个链条的包含的hook点
