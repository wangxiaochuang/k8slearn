...
func (a *APIInstaller) Install() ([]metav1.APIResource, []*storageversion.ResourceInfo, *restful.WebService, []error) {
	...
	// 遍历这个apiGroup下的所有path，并注册handler到service
...
func (a *APIInstaller) registerResourceHandlers(path string, storage rest.Storage, ws *restful.WebService) (*metav1.APIResource, *storageversion.ResourceInfo, error) {
	...
	// 制作actions list，每个resource的每一个verb一条记录
	switch {
	...
	if utilfeature.DefaultFeatureGate.Enabled(features.StorageVersionAPI) &&
		...
		// 决定放入etcd时使用的version；以及从etcd取出时可以转化为的version
		versioner := storageVersionProvider.StorageVersion()
	...
	// 根据Serializer，得出支持的MediaTypes，从而设置webservice的response中的属性
	...
	// 这个是处理HTTP请求的一个上下文，比如HttpPyaload与ExternalVersion之间的转换
	...
	// 逐个处理每一个action，基于reqScope的信息，为他们生成route并注册到webservice中去
	for _, action := range actions {
		...
		case "PUT": // Update a resource.
			...
			// restfulUpdateResource 涉及admission的处理
